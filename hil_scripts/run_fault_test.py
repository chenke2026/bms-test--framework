...
eng.eval(f"writetable(T, '{csv_name}');", nargout=0)
print(f"✅ CSV 已保存: {csv_name}")

eng.quit()
print("🔌 MATLAB 引擎已关闭。")

# 读取 CSV，模拟故障注入（在后处理中标记异常点）
print("\n📊 在后处理中模拟故障注入...")
df = pd.read_csv(csv_name)

# 在 5~25 秒之间随机插入 3~5 个信号突变点
fault_indices = random.sample(range(int(0.2*len(df)), int(0.9*len(df))), k=random.randint(3, 6))
df['Fault_Triggered'] = False
df['Fault_Desc'] = ''
for idx in fault_indices:
    df.loc[idx, 'Voltage'] = df.loc[idx, 'Voltage'] * 1.1  # 模拟电压突变
    df.loc[idx, 'Fault_Triggered'] = True
    df.loc[idx, 'Fault_Desc'] = f"信号突变 (时间 {df.loc[idx, 'Time']:.2f}s)"

# 保存带故障标记的 CSV
fault_csv = csv_name.replace('.csv', '_with_faults.csv')
